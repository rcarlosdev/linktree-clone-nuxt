    ...
    <div id="MainLinkBoxSection" class="px-8 py-5">
      <div class="flex items-center justify-between py-1">
        <div class="flex items-center w-full">
          <input
            ...
            v-model="name"
            maxlength="18"
            class="w-full text-sm font-semilbold focus:outline-none"
          />
          <div v-else class="flex items-center w-full">
            <div
              @click="
                ($event) => (
                  (name = link.name),
                  $emit('updateInput', { id: link.id, str: 'isName' })
                )
              "
            ...
            <Icon
              @click="
                ($event) => $emit('updateInput', { id: link.id, str: 'isName' })
              "
          ...
          <div v-else class="flex items-center w-[calc(100%-2px)]">
            <div
              @click="
                ($event) => (
                  (url = link.url),
                  $emit('updateInput', { id: link.id, str: 'isLink' })
                )
              "
            ...
            <Icon
              @click="
                ($event) => $emit('updateInput', { id: link.id, str: 'isLink' })
              "
      ...
      <div class="flex items-center justify-between py-1 mt-4">
        <div class="flex items-center w-full relative">
          <div
            class="flex items-center px-1.5 py-[2px] absolute -left-[6px] rounded-lg"
      ...
      <div v-if="!isMobile" class="flex items-center">
        <div
          class="flex items-center px-1.5 py-[2px] rounded-md relative w-full"
        >
          <button
...
const changeInput = (str, linkNameString) => {
  if (selectedId.value == link.value.id && selectedStr.value == str) {
    setTimeout(() => {
      document.getElementById(`edit${linkNameString}-${link.value.id}`).focus();
      return;
    }, 100);
  }
};
